...
const { isWindows, isArchive } = require('./util');
...
ipcMain.on('FETCH_ITEMS', async (event, path) => {
    ...
    try {
        ...
        if (stats.isDirectory()) {
            ...
            d(`${path} is an archive, extracting...`);
        }else {
            d(`${path} is invalid`);
            event.sender.send('FETCH_ITEMS_ERROR', { code: 'INVALID_PATH' });
        }
    }catch (err) {
        console.error(err);
        event.sender.send('FETCH_ITEMS_ERROR', err);
    }
});

ipcMain.on('OPEN_ARCHIVE', async (event, path) => {
    d(`Opening Archive ${path}`);
    try {

    }catch (err) {
        console.error(err);
        event.sender.send('OPEN_ARCHIVE_ERROR', err);
    }
});
